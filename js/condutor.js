$(document).ready(function(){
    $("#condutor-salvar").click(function(){
        if ((valCpf($("#cpf").val())) && ($("#nome").val() != "") && ($("#pgu").val() != "") && ($("#salario").val() != "") && ($("#cep").val() != "") && ($("#logradouro").val() != "") && ($("#numero").val() != "") && ($("#bairro").val() != "") && ($("#estado").val() != "") && ($("#cidade").val() != ""))  {
            $("#cidade").attr("disabled",false);
            $("#estado").attr("disabled",false);
            if ($("#acao").val()=="CADASTRAR"){
                $("#acao").val("SALVARCADASTRO");
                var dados = $("#condutor").serialize();
                $("#modal").show();
                $.ajax({
                    url: "salvar_condutor.php",
                    type: "post",
                    dataType: "json",
                    data: dados,
                    success: function(result){
                        ...
                        setTimeout(function(){
                            $("#alert").hide();
                            window.location="visu_condutor.php";
                        },2000);
                    }
                });
            }
            ...
            if (!($("#pgu").val() != "")) {
               $("#pgu-form").addClass("has-error");
               erro = 1;
            } else {
                $("#pgu-form").removeClass("has-error");
            }
            if (!($("#nome").val() != "")) {
